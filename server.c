...
int tcp_sock, udp_sock, connfd, fd, bytes_read, tcp_port, payload_size, udp_port, k, i, j, x, flag;
float lambda, Q, Q_star, Gamma, fl_a, fl_delta, fl_epsilon, fl_beta;
float ** control_param_list;
FILE * mode_fd;
char ip[15], logfile[100], filename[100], tmp[10], str[10], cli_udp_port[10], a[5], delta[5], epsilon[5], beta[5], buf[100], feedback[200];
char tmp_path[110];
int mode;
...
int main(int argc, char** argv) {
	...
	while(1){
		...
		if (fd < 0){ // File does not exist on server
			...
			printf("File Exists!!\n");
			write(connfd, "2", 1); // Send 2 back to client
			...
			cli_addr.sin_port = htons(atoi(cli_udp_port));
			...
			if (k==0) {
				...
				while((read_file = read(fd, audio_data, payload_size)) > 0){
					...
					sendto(udp_sock, (const char *)audio_data, sizeof(audio_data), 0, (const struct sockaddr *) &cli_addr, sizeof(cli_addr));
					usleep(1/lambda);
...
void client_feedback_handler(){
	socklen_t cli_addr_size = sizeof(cli_addr);
	bytes_read = recvfrom(udp_sock, (char *)feedback, sizeof(feedback), 0,
		 (struct sockaddr *) &cli_addr, &cli_addr_size);
	
	// Copy first four bytes into Q
	memcpy(&Q, feedback, 4);
	// Copy next four bytes into Q_Star
	memcpy(&Q_star, feedback + 5, 4);
	// Copy next four bytes into gamma
	memcpy(&Gamma, feedback + 9, 4);
...
void createUDP(){
	...
	// Bind the socket with the server address
	if ( bind(udp_sock, (const struct sockaddr *)&udp_servaddr, sizeof(udp_servaddr)) < 0 ){
		perror("bind failed");
		exit(EXIT_FAILURE);
	}
